...
// Quo SMS API (https://quoapp.io)
// Set QUO_API_KEY and QUO_FROM_NUMBER in Vercel env vars
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (requestId) await supabase.from('sms_logs').insert({ request_id: requestId, phone, message, type, success: result.success, sms_id: result.id || null, sent_by: user.id })
...
export async function GET(req: NextRequest) {
  ...
  const { data } = await supabase.from('sms_logs').select('*').eq('request_id', requestId || '').order('created_at', { ascending: false })
