    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-slate-950">
      <div className="w-full max-w-md">
        ...
        <div className="card space-y-4">
          ...
          <button onClick={handleLogin} disabled={loading} className="btn-primary w-full py-3 mt-2">{loading?'Signing in...':'Sign In'}</button>
          <p className="text-center text-slate-400 text-sm">Don&apos;5 have an account?{' '}<Link href="/auth/signup" className="text-sky-400 hover:text-sky-300 font-medium">Sign up</Link></p>
        </div>
      </div>
    </div>
  )
