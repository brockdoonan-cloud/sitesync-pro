...
export async function middleware(request: NextRequest) {
  ...
  const botPatterns = /curl|wget|python-requests|go-http|scrapy|java\/|libwww|perl|ruby|php|bot(?!.*chrome)|spider|crawl/i
  if (botPatterns.test(ua) && (path.startsWith('/auth/') || path.startsWith('/dashboard/')))
  ...
  if(!user&&path.startsWith('/dashboard')){const url=request.nextUrl.clone();url.pathname='/auth/login';return NextResponse.redirect(url)}
  if(user&&path.startsWith('/auth')&&!path.includes('/callback')){const url=request.nextUrl.clone();url.pathname='/dashboard';return NextResponse.redirect(url)}
